...
def get_recommendations():
    ...
    ranking_queries = [
        ...
        for (u, m) 
        in zip([user_id] * NUM_OF_CANDIDATES, movie_candidates)
    ]
    ...
    movie_scores = list(
        np.squeeze(ranking_response.json()["predictions"])
        )
    
    ranked_movies = [
        m[1] for m in sorted(list(zip(movie_scores, movie_candidates)),
                             reverse=True)
    ]
